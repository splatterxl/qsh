...
export function parseFlags<F extends Record<string, FlagSchema>>(
  ...
  parsed: ParsedFlags<F>,
  invalid: Record<string, string | null>,
  unrecognised: string[],
  rest: string
] {
  ...
  function saveFlag(char?: string, lastSave = false): void {
    ...
    if (!flag)
      ...
      if (
        (flag.validate && !flag.validate(currentValue)) ||
...
export type ParsedFlags<Flags extends Record<string, FlagSchema>> = {
  -readonly [key in keyof Flags]: Flags[key] extends FlagParam
    ? ParsedParameter<Flags[key]>
    : boolean;
};

type ParsedParameter<Param extends FlagParam> =
  Param['choices'] extends ReadonlyArray<infer T>
...
type PossiblyOptionalParam<
  Param extends FlagParam,
  T = string
> = Param['optional'] extends true ? T | true : T;
