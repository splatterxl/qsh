...
function parse(file) {
    const result = (0, parser_1.parseFile)(file);
    if (result[1].length !== 0) {
        for (const err of result[1]) {
            console.log((0, parser_1.formatError)(err, result[2], file));
        }
        console.log('\n' +
            (0, chalk_1.default) `{yellow ${result[1].length}} error${result[1].length > 1 ? 's' : ''} found.`);
        return false;
    }
    else
        return result;
